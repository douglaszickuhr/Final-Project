...
## Fuzzy Clustering
```{r}
companies_fcm <- FKM(companies_with_pca %>%
                  select(starts_with("PC")),
         k = 5,
         RS = 20,
         seed = 123)
...
companies_clusters <- companies_with_pca %>%
  ...
  cbind(fuzzy_cluster = companies_fcm$clus) %>%
  dplyr::rename(fuzzy_cluster = `fuzzy_cluster.Cluster`,
         fuzzy_cluster_degree = `fuzzy_cluster.Membership degree`)
```
